...
const ContactDetails = ({ currentId, handleEditContact, nameAvatar }) => {
  ...
  const contactPhones = contact?.phones?.filter(
    (phone) => phone.id !== deletedPhones
  );
      ...
      {contact && currentId ? (
        <>
          <div className={classes.head}>
            <Avatar
              {...nameAvatar(contact?.name || contact?.email)}
          ...
          <div className={classes.detailsContainer}>
            <div className={`${classes.details} contact-info`}>
              ...
              <div style={{ display: "flex", flexFlow: "column" }}>
                ...
                {contactPhones?.map((phone) => (
                  <div
                    key={phone.id}
                    className={`${classes.phone} contact-item `}
                  >
                    <MdCall
                      style={{
                        fontSize: "20px",
                    ...
                    {openMenu && (
                      <div className={classes.icons}>
                        <div
                          className="icon"
                          onClick={() => copyText(phone.value)}
                        >
                          <MdContentCopy />
                        </div>
                        <div
                          className="icon"
                          onClick={() => deletePhones(phone.id)}
                        >
                          <MdDelete />
                        </div>
                        <div
            ...
            <div className={classes.notes}>
              ...
              {contact?.notes ? (
                <Typography style={{ marginBlock: "5px" }}>
                  {contact.notes}
                </Typography>
              ) : (
                <Button
        ...
        <Paper elevation={6} className={classes.loadingPaper}>
          <Typography>
            Click at contact to see details of the contact
          </Typography>
        </Paper>
      )}
    </div>
